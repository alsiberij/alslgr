...
// with []byte and uses default alslgr.SliceProducer (which is [][]byte) for batching. Keep in mind that this instance
// uses only one worker in order to achieve consequential writing.
...
func NewBatchedWriter(batchMaxLen int, maxBufferLenBeforeWriting int, filename string,
	lastResortWriter io.Writer) (bw alslgr.BatchedWriter[[][]byte, []byte], err error) {

	bp := alslgr.SliceProducer[[]byte](batchMaxLen)

	w, err := newWriter(filename, lastResortWriter, maxBufferLenBeforeWriting)
	if err != nil {
		return bw, err
	}
